...
+
Der Storage Automation Store enthält NetApp-zertifizierte automatisierte Storage-Workflow-Pakete, die für die Verwendung mit OnCommand Workflow Automation (WFA) entwickelt wurden. Die Pakete können heruntergeladen und anschließend in WFA importiert werden, um sie auszuführen. Die automatisierten Workflows sind im Storage Automation Store verfügbar.
